// ===================================================================================
// Project:   Tiny Tris and Tiny Pacman - Conversion for CH32V003
// Version:   v1.1
// Year:      2024
// ===================================================================================
//
...
//  Tiny-tris and Tiny-pacman are free software: you can redistribute it and/or modify
...
// Modified to include both Tiny Tris and Tiny Pacman by annoyedmilk: https://github.com/annoyedmilk
// ===================================================================================
...
// ===================================================================================
static void handle_tetris_game(uint8_t *state);
static void handle_pacman_game(uint8_t *state);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
    case MENU_STATE:
      handle_menu(&state, &current_game);
      break;

    case TETRIS_STATE:
      handle_tetris_game(&state);
      break;

    case PACMAN_STATE:
      handle_pacman_game(&state);
      break;
    }
  }
}
  ...
  while (*state == PACMAN_STATE)
  {
    ...
    while (*state == PACMAN_STATE)
    {
      ...
      if (LEVELSPEED > 10)
      {
        ...
        if ((LEVELSPEED == 160) || (LEVELSPEED == 120) ||
            (LEVELSPEED == 80) || (LEVELSPEED == 40) || (LEVELSPEED == 10))
        {
          if (LIVE < 3)
          {
            LIVE++;
      ...
      // Reset game state and initialize sprites
      Gobeactive = 0;
      ...
      while (*state == PACMAN_STATE && !level_complete)
      {
        if (JOY_act_pressed())
          StartGame(&Sprite[0]);
        ...
        Frame = (Frame < 24) ? Frame + 1 : 0;

        // Check collisions and update characters
