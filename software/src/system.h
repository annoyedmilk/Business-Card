...
#ifndef F_CPU
#define F_CPU 12000000 // 24Mhz if not otherwise defined
#endif
...
#define AWU_set(ms) \
  (ms < 64 ? ({PWR->AWUPSC = 0b1000; PWR->AWUWR = (ms); }) : (ms < 128 ? ({PWR->AWUPSC = 0b1001; PWR->AWUWR = (ms)>>1; }) : (ms < 256 ? ({PWR->AWUPSC = 0b1010; PWR->AWUWR = (ms)>>2; }) : (ms < 512 ? ({PWR->AWUPSC = 0b1011; PWR->AWUWR = (ms)>>3; }) : (ms < 1024 ? ({PWR->AWUPSC = 0b1100; PWR->AWUWR = (ms)>>4; }) : (ms < 2048 ? ({PWR->AWUPSC = 0b1101; PWR->AWUWR = (ms)>>5; }) : (ms < 5120 ? ({PWR->AWUPSC = 0b1110; PWR->AWUWR = (ms)/80; }) : (ms < 30720 ? ({PWR->AWUPSC = 0b1111; PWR->AWUWR = (ms)/480; }) : (0)))))))))
